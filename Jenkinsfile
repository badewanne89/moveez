            ...
            stages {
                stage('DEPLOY') {
                    steps {
                        ...
                        sh "docker run -p 443 --log-driver=gelf --log-opt gelf-address=udp://0.0.0.0:12201 --name ${packageJSON.name}_uat_${env.RELEASE_NAME} -e NODE_ENV='uat' -d ${env.DOCKER_IMAGE_NAME}_rc"
                        script {
